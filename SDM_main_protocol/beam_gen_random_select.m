function [ d_s] = beam_gen_random_select(RX_measure_report, Beam_index_report)
            ...
            if(isempty(candi_set) == 0)
                ...
                if(isempty(best_beams_rem)==0)
                    for b=1:1:length(best_beams_rem)
                        this_beam = rem_candi(Beam_index_report(Nap_bw,rem_candi) == best_beams_rem(b));
                        %rate = 1e-6*DataRate(min(RX_log_measure_report(q,this_beam)))
                        loc_min = min(RX_measure_report(Nap_bw,this_beam));
