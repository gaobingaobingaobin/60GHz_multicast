function [d_s] = beam_gen_strongest_first(RX_measure_report, RX_log_measure_report,Beam_index_report,k)
%BEAM GROUP GENERATION SELECTING Strongest CLIENT FIRST
    ...
    while(sum(served) < N_u)
       ...
       beam_count = beam_count + 1;
        ...
        %Finding the main client (i.e. the client with Strongest signal strength
        %at lowest codebook level = k
        served_locs = find(served == 1);
        unserved_locs = find(served == 0);
        [~,ind] = max(squeeze(RX_log_measure_report(k+1,unserved_locs)));
        main_client = unserved_locs(ind);
