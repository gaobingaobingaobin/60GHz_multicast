function [SNR,SINR]=A_BF_SIM_1(TX_location,RX_location,TX_rotation,RX_rotation,room_size,M,K,Pt)
...
for link=1:N_a
    ...
    for TX_beam_direction=1:K %exaustive search all beam pairs
        for RX_beam_direction=1:K
            ...
            [current_SNR]=get_SNR(Pt,PN,M,lamda,TX_location(link,:),RX_location(link,:),TX_rotation(link),RX_rotation(link),room_size,RX_beam_vector(:,link),TX_beam_vector(:,link));
            SNR_matrix(TX_beam_direction,RX_beam_direction)=current_SNR;
        end
    end
    
    max_SNR=max(max(SNR_matrix)); %find the beam pair that achieves the highest SNR
    [TX_beam_direction_opt,RX_beam_direction_opt]=find(SNR_matrix==max_SNR);
    TX_beam_direction_opt=TX_beam_direction_opt(1);
    RX_beam_direction_opt=RX_beam_direction_opt(1);
    
    TX_beam_vector(:,link)=W(:,TX_beam_direction_opt); %tramsimter's beamforming weight vector
    RX_beam_vector(:,link)=W(:,RX_beam_direction_opt); %receiver's beamforming weight vector
    SNR(link)=max_SNR;
    
end

%==================== obtain signal power and interference power for each RX ====================
for TX=1:N_a
    for RX=1:N_a
        % obtain channel matrix H for each two node pair
